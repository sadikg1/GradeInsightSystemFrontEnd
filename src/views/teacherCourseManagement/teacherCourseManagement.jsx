...
const TeacherCourseManagement = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      await getData("/TeacherxCourses").then((res) =>
        setTeacherxCourses(res.data)
      );
  ...
  const handleSubmit = async (values) => {
    try {
      if (isEdit && editCardId) {
        const updatedData = await putData(`/TeacherxCourses/${editCardId}`, {
          ...values,
          teacherxCourseId: editCardId,
        });
        ...
        setTeacherxCourses((prevState) => {
          return prevState.map((item) =>
            item.teacherxCourseId === editCardId
      ...
      >
        <div>
          ...
          <div style={{ marginLeft: "20px", marginTop: "7px" }}>
            Total Teacher-Course: {teacherxCourse.length}
          </div>
        </div>
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12}>
                    ...
                    >
                      <Button
                        type="submit"
                        style={{ background: "#1d396e", color: "white" }}
